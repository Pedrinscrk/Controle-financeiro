...
const generateID = () => Math.round(Math.random()*1000); // gera o id
...
form.addEventListener('submit', event => {
  ...
  if(transName === '' || transAmount === '' ) {
    alert('Informe tanto o nome quanto o valor da transação!!!');
    return
  }
  const transaction = {
    id: generateID(), 
  ...
  console.log(transaction); // retona a transação
  ...
  inputTransactionAmount.value = ''; // 0 o input
  inputTransactionName.value = ''; // limpa a transação
})
